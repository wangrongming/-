...
async def taobao(username, password, url, page):
    ...
    if slider:
        ...
        try:
            ...
            logger.info("error_1:", error)
            error = await page.Jeval('.error', 'node => node.textContent')
            logger.info("error_2:", error)
        except Exception as e:
            error = None
        finally:
            if error:
                logger.info('确保账户安全重新入输入')
            else:
                page = await go_kf_page(page)
                result = await get_cookie(username, page, url)
                return result if result else None
...
async def get_cookie(username, page, url):
    ...
    logger.info('cookie页面的url' + page.url)
