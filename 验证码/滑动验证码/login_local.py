...
async def taobao(username, password, url, page):
    ...
    while True:
        if 'login' in page.url:
            ...
            offsets, tracks = await get_tracks(await get_gap_distance() * 278 / 360, 3, 'ease_out_expo')
            # 因为hover滑块之后鼠标在滑块中间,这个时候down之后,鼠标在滑块27像素之前移动都无法造成滑块移动 # TODO
            lenth = i['x'] + 27
            high = i['y']
...
async def get_gap_distance():
    otemp = 'small_code.png'
    oblk = 'code.png'

    # 读取图片
    small_code = cv2.imread(otemp, 0)
    code = cv2.imread(oblk, 0)
...
async def download_pic(page):
    ...
    pic_bs64_smaller = re.findall(r'<img src="data:image/png;base64,(.*?)"', page_html)[1]
    image_data = base64.b64decode(pic_bs64_smaller)
    with open('small_code.png', 'wb') as f:
        f.write(image_data)
