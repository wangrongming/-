...
import redis_block  # redis模块

server = SSHTunnelForwarder(
    ssh_address_or_host=('115.159.127.155', 22),  # ssh地址
    ssh_username='root',  # ssh连接的用户名
    ssh_password='TLkxPI6B',  # ssh连接的用户名
    remote_bind_address=('10.1.6.8', 6379),
    local_bind_address=('127.0.0.1', 10022))

with server:
    start_time = time.time()
    r = redis_block.Redis(host='127.0.0.1', port=10022, db=3, password="kaVd9*F(Ax#q",
                          decode_responses=True)  # decode_responses: 解码
